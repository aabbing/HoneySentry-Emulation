...
function run_emulation()
{
    ...
    if [ ! "${ARCH}" ]; then
        ...
        echo "get architecture fail" > ${WORK_DIR}/resultf
        return
    fi
    ...
    if [ ${OPTION} = "analyze" ]; then
        ...
        # ================================
        if ($PING_RESULT); then
            sudo ip addr flush dev tap${IID}_0 &
            echo -e "[\033[32m+\033[0m] Run debug!"
            sudo kill $(ps aux | grep socat | awk '{print $2}') 2> /dev/null | true
            IP=`cat ${WORK_DIR}/ip`
            socat TCP-LISTEN:80,reuseaddr,fork TCP:${IP}:80 &
            socat TCP-LISTEN:23,reuseaddr,fork TCP:${IP}:23 &
            echo -e "[\033[32m+\033[0m] Run Route!"
            sudo rm ./scratch/${IID}/run_debug.sh
            sudo ln -s ./run.sh ./scratch/${IID}/run_debug.sh
            sudo chmod +x ./scratch/${IID}/run.sh
        ...
        # ================================
        sudo ip addr flush dev tap${IID}_0 &
        echo -e "[\033[32m+\033[0m] Run!"
        sudo kill $(ps aux | grep socat | awk '{print $2}') 2> /dev/null | true
        IP=`cat ${WORK_DIR}/ip`
        socat TCP-LISTEN:80,reuseaddr,fork TCP:${IP}:80 &
        socat TCP-LISTEN:23,reuseaddr,fork TCP:${IP}:23 &
        sudo rm ./scratch/${IID}/run_boot.sh
        sudo ln -s ./run.sh ./scratch/${IID}/run_boot.sh
        sudo chmod +x ./scratch/${IID}/run.sh 
        check_network ${IP} false &
        ${WORK_DIR}/run.sh 
